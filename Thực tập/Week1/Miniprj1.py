...
def translate_text(text_to_translate, target_language):
    prompt = f"Dịch đoạn văn bản sau sang {target_language} một cách chính xác và tự nhiên, giữ nguyên ý nghĩa gốc:\n\n{text_to_translate}"
...
def main():
    while True:
        ...
        print("2. Dịch văn bản (mọi ngôn ngữ)")
        ...
        if choice == '1':
            ...
            lang = input("Bạn muốn dịch sang ngôn ngữ nào? (VD: English, Japanese, French...): ")
            if not lang.strip():
                print("❌ Bạn phải nhập ngôn ngữ đích!")
                continue
